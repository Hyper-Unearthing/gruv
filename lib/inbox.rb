...
class Inbox
  ...
  def next_pending(processing_timeout_seconds: 300)
    ...
    media_table_attachments = ChannelAttachment.order(:id).map { |attachment| attachment_payload(attachment) }

    {
      ...
      media_table_attachments: media_table_attachments,
  ...
  def insert_channel_attachments(message_record:, metadata:)
    ...
    attachment_files = metadata[:attachment_files] || []
    attachment_files.each do |attachment|
      ...
      ChannelAttachment.create!({
        ...
        attachment_type: attachment[:type] || 'attachment',
        provider_file_id: attachment[:file_id] || attachment[:id],
    ...
    image_file_paths = metadata[:image_file_paths] || []
    ...
    photo_file_paths = metadata[:photo_file_paths] || []
